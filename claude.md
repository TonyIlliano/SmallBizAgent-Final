...
#### Website Builder Fixes — Booking iframe, Hours Nudge, HTML Editor
- **Goal**: Fix booking page not loading in generated sites, fix false "Add Hours" nudge when hours already exist, add ability to directly edit generated HTML.
...
- `client/src/pages/website-builder.tsx` — **HTML Editor**: Added collapsible "Edit HTML" card with toggle button. Full-width monospace textarea for direct HTML editing. "Save Changes" button saves via `PUT /api/website-builder/site`. "Revert" button resets to last saved version. Preview iframe auto-refreshes after save via `previewKey` counter. Editor content reloads after regeneration.
...
*Last updated: March 20, 2026. 345 tests passing (227 unit + 118 E2E). Zero TypeScript errors. 62 tables. Website Builder overhaul (uncommitted): Replaced Google Stitch SDK with direct OpenAI generation (gpt-5.4-mini). Business scanner service (scrape URL/Google Business Profile → OpenAI extraction → structured data). Website generation service (OpenAI gpt-5.4-mini with comprehensive system prompt, 15+ vertical design presets, dynamic DB data, booking widget embedding, customization support). Custom domain management (subdomain auto-gen + custom CNAME + DNS verification + purchase stub). Website storage/serving (websites table, /sites/:subdomain public serving, branded 404). Plan-based feature gates (Starter: subdomain only, Professional: custom domain, Elite: managed setup). Customization panel (accent color, font style, hero headline/image, section toggles). Profile nudges (missing services/staff/hours warnings). New files: websiteGenerationService.ts. Deleted: stitchService.ts. Modified: businessScannerService.ts, websiteBuilderRoutes.ts, website-builder.tsx, schema.ts (websites table: removed stitchPrompt, added customizations + generatedAt). Removed dep: @google/stitch-sdk. Removed env var: STITCH_API_KEY. Website Builder fixes (uncommitted): Booking iframe URL now uses APP_URL dynamically. /sites/ route CSP headers fixed for dashboard preview embedding. Hours nudge fixed (was querying wrong endpoint). HTML editor added for direct code editing with save/revert/preview refresh.*
