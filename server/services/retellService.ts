  ...
  tools.push(customTool(
    ...
    'Book after customer confirms. Use dateForBooking from checkAvailability.',
    {
      ...
      properties: {
        ...
        urgency: { type: 'string', enum: ['emergency', 'urgent', 'routine'], description: 'How urgent the job is: emergency (no heat/water, safety risk), urgent (needs attention soon), or routine (standard scheduling). Only set if the caller indicates urgency.' },
        issueType: { type: 'string', description: 'Short category of the problem, e.g. "AC not cooling", "water heater leak", "no heat".' },
        symptoms: { type: 'string', description: 'What the customer is experiencing in their own words, e.g. "making a grinding noise and blowing warm air".' },
        accessNotes: { type: 'string', description: 'How the tech should access the property, e.g. "gate code 1234", "dog in backyard", "unit on the roof".' },
      },
